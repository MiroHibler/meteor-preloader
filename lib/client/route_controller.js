	...
	PreloadController = RouteController.extend({
		...
		onRun	: function () {
			...
			if ( !self.router._async ) {
				self.router._async = {
					js: ( preload && preload.async && preload.async.js ) ? preload.async.js || [] : []
				};
			}
		},
		...
		waitOn	: function () {
			var self			= this,
				...
				commonLibs		= ( routerLibs && routerLibs.sync && routerLibs.sync.common ) ? routerLibs.sync.common || {} : {},
				defaultLibs		= ( routerLibs && routerLibs.sync && routerLibs.sync.default ) ? routerLibs.sync.default || {} : {},
				preloadHandlers	= [];
