...
var verbose = false,
	setVerbose = function ( self ) {
		verbose = (
			self.router.options.preload &&
			self.router.options.preload.verbose &&
	...
	Console = {
		info : function ( params ) {
			if ( verbose ) console.info( params );
		},

		warn : function ( params ) {
			if ( verbose ) console.warn( params );
		},

		error: function ( params ) {
			if ( verbose ) console.error( params );
		}
	};
		...
		var self          = this,
			...
			loadStyles = function () {
				var absoluteUrl = Meteor.absoluteUrl(),
					routeStyles = _.map( thisRoute._libs.sources.styles, function ( link ) {
						if ( _.isArray( link ) ) {
							return _.map( link, function ( singleLink ) {
								return singleLink.replace( absoluteUrl, '/' );
							});
						} else {
							return link.replace( absoluteUrl, '/' );
						}
					}),
					currentStyles = _.map( $( 'link[ rel*=preload ]' ), function ( link ) {
						return link.href.replace( absoluteUrl, '/' );
					}),
					allStyles = _.union( currentStyles, routeStyles ),
					excessStyles = _.difference( allStyles, routeStyles ),
					retainedStyles = _.difference( currentStyles, excessStyles ),
					newStyles = _.difference( routeStyles, retainedStyles );

				// Remove excess styles
				if ( excessStyles.length ) {
					_.map( excessStyles, function ( style ) {
						$( 'link[ href="' + style + '" ]' ).remove();

						Console.warn( '[ Preloader - Styles ] Removing: ' + style );
					});
				}
				...
				if ( newStyles.length ) {
					...
					$.map( newStyles, function ( styleFile ) {
						styleCounter++;

						$( 'head' ).append( '<link rel="preload stylesheet" type="text/css" href="' + styleFile + '"/>' );

						Console.info( '[ Preloader - Styles ] Loading ' + styleCounter + '/' + newStyles.length + ': ' + styleFile );
					});
...
if ( typeof PreloadController === 'undefined' ) {
	PreloadController = RouteController.extend({
		waitOn: function () {
			...
			return Preloader.loadLibs( self );
		},
